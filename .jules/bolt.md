...
## 2025-02-12 - Redundant Image Decoding inside Loops

**Learning:** When performing batch operations on images (like iterative overlap detection during smart stitching), redundantly loading and decoding the same image source into an `HTMLImageElement` across multiple loop iterations severely degrades client-side processing performance.
**Action:** Always persist/cache the parsed image reference from the previous loop iteration (e.g., `let prevImg = imgB`) outside of the block scope to halve the total number of asynchronous decoding operations (`loadImage` calls).
