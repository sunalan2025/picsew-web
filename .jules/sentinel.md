...
## 2024-08-25 - [Fix] Image Bomb / Pixel Flood DoS Vulnerability
**Vulnerability:** The application was vulnerable to Image Bomb / Pixel Flood DoS attacks because it only checked the compressed file size (e.g., 50MB) and not the uncompressed pixel footprint. A small malicious image file (e.g., a highly compressed 10000x10000 pixel image) could easily pass the file size check but consume massive amounts of memory (hundreds of MBs) when decoded and drawn to a Canvas, leading to browser crashes or Out-Of-Memory (OOM) errors.
**Learning:** File size limits are insufficient to prevent OOM DoS attacks in Canvas-heavy applications. The true memory footprint of an image is dictated by its uncompressed dimensions (width * height * bytes per pixel), not its compressed disk size.
**Prevention:** Always validate the uncompressed pixel footprint (`img.naturalWidth * img.naturalHeight`) of uploaded images inside the `img.onload` handler before processing them in memory or rendering them to a `<canvas>`. Set a reasonable maximum pixel limit (e.g., 25,000,000 pixels) based on the application's use case and client device memory constraints.
